...
def pearson(v1, v2):
    ...
    numerator = p_sum - (sum1 * sum2) / len(v1)
    denumerator = math.sqrt(((sum1_sq - ((sum1 ** 2) / len(v1))) * (sum2_sq - (sum2 ** 2) / len(v1))))
    if denumerator == 0:
        return 0
...
class BiCluster(object):
    def __init__(self, vec, left=None,
                 right=None, distance=0.0,
                 id=None):
        self.left = left
        self.right = right
        self.vec = vec
...
def hcluster(rows, distance=pearson):
    ...
    """对clust进行聚类，将距离最近的两个点合并成一个新的节点
    为了计算的简便，将距离的中间结果进行保存"""
    while len(clust) > 1:
        closest_pair = (0, 1)
        closest_distance = pearson(clust[0].vec, clust[1].vec)
        ...
        merge_vec = [(clust[closest_pair[0]].vec[k] + clust[closest_pair[1]].vec[k]) / 2
                     for k in range(len(clust[0].vec))
                     ]
        new_cluster = BiCluster(merge_vec, left=clust[closest_pair[0]],
                                right=clust[closest_pair[1]],
...
if __name__ == '__main__':
    ...
    # clust = hcluster(data)
    # print clust
    # drawdendrogram(clust, blognames, jpeg='blog.jpg')
